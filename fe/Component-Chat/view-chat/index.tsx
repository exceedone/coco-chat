...
const MeetingChat = ({ meetingInfo }: IMeetingChat) => {
  ...
  const [initStatus, setInitStatus] = useState<FETCH_STATUS>(FETCH_STATUS.IDLE);
        ...
        setDataChat((prev) => {
          return {
            ...
            messageChat: [
              ...
              {
                ...message,
                receiverId: message.receiverId ? message.receiverId : 0,
              },
            ],
          };
        });
  ...
  useEffect(() => {
    const fetchDataChat = async (meetingId: number) => {
      ...
      try {
        const res = await serviceChatMeeting.getAllMessageChatByMeetingId(
          meetingId
        );
        if (res) {
          setDataChat(res);
                  ...
                  <div className="border-black-500 h-full overflow-hidden border px-2 hover:overflow-y-auto">
                    {dataChat?.messageChat.map((message, index) => {
                      ...
                      const dateInfo = {
                        ...
                        month: dateMessage.getUTCMonth() + 1,
                      ...
                      if (index == 0) {
                        ...
                        const dateMessagePrev = new Date(
                          dataChat.messageChat[index - 1].createdAt
                        );
                        dateInfoPrev = {
                          year: dateMessagePrev.getUTCFullYear(),
                          month: dateMessagePrev.getUTCMonth() + 1,
                          day: dateMessagePrev.getUTCDate(),
                        };
                      }

                      if (message.sender.id == authState.userData?.id) {
                        return (
                          <MessageChatItemFromYou
                            key={index}
                            ...
                            messageInfoPrev={{
                              from: dataChat.messageChat[index - 1].sender
                                .email,
                              to: dataChat.messageChat[index - 1].receiver
                                .email,
                              datePrev: {
                                ...dateInfoPrev,
                              },
                      ...
                      return (
                        <MessageChatItemToYou
                          ...
                          message={message.content}
                          messageInfoPrev={{
                            from: dataChat.messageChat[index - 1]?.sender.email,
                            to:
                              dataChat.messageChat[index - 1]?.receiver.email ==
                              authState.userData?.email
                                ? "You"
                                : dataChat.messageChat[index - 1]?.receiver
                                    .email,
                            datePrev: {
                              ...dateInfoPrev,
                            },
                  ...
                  <div className="flex w-[95%] flex-col gap-2">
                    <div>
                      ...
                      <Select
                        ...
                        options={participantToSendMessage.map((user) => ({
                          value: user.userId + "",
